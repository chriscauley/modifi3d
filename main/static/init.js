...
$(document).ready(function() {
  ...
  setTimeout( function(){$('#editorWrapper').css('right','-900px');},3000); // -- hide slide-editor after 3secs
...
if(me=='web-online') {
  ...
  for(var i=0; i<ex.length; i++) {
    ...
    src += "<li><a href='#"+ex[i].file.slice(1)+"' onclick='fetchExample(\""+ex[i].file+"\");'>"+ex[i].title+"</a>\n";
    if(ex[i].type) src += " <span class=type>("+ex[i].type+")</span></a>";
    if(ex[i].new) src += " <span class=newExample>new</span></a>";
    //src += "<li><a href='"+ex[i].file+"\'>"+ex[i].title+"</a>\n";
  }
  src += "</td></tr></table>";
  $('#examples').html(src);
} else {
...
function onload() {
  ...
  if(me=='web-online') {    // we are online, fetch first example
    ...
    if(document.URL.match(/#(http:\/\/\S+)$/) || document.URL.match(/#(https:\/\/\S+)$/)) {
      // remote file referenced, e.g. http://openjscad.org/#http://somewhere/something.ext
      var u = RegExp.$1;
    ...
    } else if(document.URL.match(/#(jscad\/item\/\S+)$/)) {    // local example, e.g. http://openjscad.org/#examples/example001.jscad
      var fn = RegExp.$1;
      fetchExample(fn);
      //document.location = document.URL.replace(/#.*$/,'#');
      
    } else {
      fetchExample(ex[0].file);
    }
  } else {
    gProcessor.setJsCad(editor.getValue());
  }
